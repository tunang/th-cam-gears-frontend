...
import { X, ShoppingBag, Trash2, Plus, Minus, Loader2, ImageOff } from 'lucide-react';
import { useCart, useUpdateCartItem, useRemoveCartItem } from '@/src/hooks/use-cart';
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        >
          ...
          >
            <Dialog.Title className="flex items-center gap-2 text-lg font-semibold" style={{ color: 'var(--p-color-text)' }}>
          ...
          <div className="flex-1 overflow-y-auto">
            {!isAuthenticated ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 p-8 text-center">
                <ShoppingBag className="h-14 w-14" style={{ color: 'var(--p-color-icon-secondary)' }} />
                <p className="text-base font-medium" style={{ color: 'var(--p-color-text)' }}>
              ...
              <div className="flex items-center justify-center h-full">
                <Loader2 className="h-7 w-7 animate-spin" style={{ color: 'var(--p-color-text-secondary)' }} />
              </div>
            ) : items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 p-8 text-center">
                <ShoppingBag className="h-14 w-14" style={{ color: 'var(--p-color-icon-secondary)' }} />
              ...
              <div className="flex flex-col divide-y" style={{ borderColor: 'var(--p-color-border-secondary)' }}>
                {items.map((item) => (
          ...
          {isAuthenticated && items.length > 0 && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xl font-bold" style={{ color: 'var(--p-color-text)' }}>
...
function CartItemRow({ item }: { item: CartItem }) {
  ...
  return (
    <div className={`flex gap-3 p-4 transition-opacity ${isUpdating ? 'opacity-50' : ''}`}>
      {/* Thumbnail */}
      <Link href={`/products/${variant.product.id}`} className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg" style={{ background: 'var(--p-color-bg-surface-secondary)' }}>
        {variant.product.thumbnailUrl ? (
          <Image src={variant.product.thumbnailUrl} alt={variant.product.name} fill className="object-cover" sizes="64px" />
        ) : (
          <div className="flex h-full items-center justify-center">
            <ImageOff className="h-5 w-5" style={{ color: 'var(--p-color-icon-secondary)' }} />
          </div>
        )}
      </Link>
      ...
      <div className="flex flex-1 flex-col gap-1 min-w-0">
        <Link href={`/products/${variant.product.id}`}>
          <p className="text-lg font-semibold leading-tight" style={{ color: 'var(--p-color-text)' }}>
            {variant.product.name}
          </p>
        </Link>
        <p className="text-sm font-italic leading-tight" style={{ color: 'var(--p-color-text)' }}>
            {variant.name}
          </p>
        <p className="text-sm font-bold" style={{ color: 'var(--p-color-text)' }}>
          {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(Number(variant.price))}
        </p>
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          >
            ...
            <span className="w-7 text-center text-sm font-medium tabular-nums" style={{ color: 'var(--p-color-text)' }}>
              {item.quantity}
            </span>
            <button
              onClick={() => handleQtyChange(1)}
