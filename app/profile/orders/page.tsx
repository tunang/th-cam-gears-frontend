...
import { useMyOrders, useOrderPayment, type CheckoutResult } from '@/src/hooks/use-orders';
...
export default function MyOrdersPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 md:p-8">
      <h1 className="mb-6 text-2xl font-bold" style={{ color: 'var(--p-color-text)' }}>
        Đơn hàng của tôi
      </h1>

      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed py-16 text-center" style={{ borderColor: 'var(--p-color-border)' }}>
          <PackageOpen className="mb-4 h-16 w-16" style={{ color: 'var(--p-color-icon-secondary)' }} />
          <h2 className="mb-2 text-lg font-semibold" style={{ color: 'var(--p-color-text)' }}>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs" style={{ color: 'var(--p-color-text-secondary)' }}>
                  ...
                  {order.items.map((item) => (
                    <div key={item.id} className="flex items-center gap-4">
                      ...
                      <div className="flex flex-1 flex-col min-w-0">
                        ...
                        <p className="text-xs font-medium" style={{ color: 'var(--p-color-text-secondary)' }}>
                          x {item.quantity}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold text-sm" style={{ color: 'var(--p-color-text)' }}>
                ...
                >
                  <div>
                    <p className="text-xs" style={{ color: 'var(--p-color-text-secondary)' }}>Tổng tiền</p>
                    <p className="text-lg font-bold" style={{ color: 'var(--p-color-text-critical)' }}>
      ...
      {data?.meta && data.meta.lastPage > 1 && (
        <div className="mt-8 flex justify-center gap-2">
          <button
            ...
            style={{ borderColor: 'var(--p-color-border)', color: 'var(--p-color-text)' }}
          >
            Trang trước
          </button>
